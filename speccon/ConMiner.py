...
from speccon.invariantslice import InvariantSlice
from speccon.PrefixTree import PrefixTree, DataPrefixTree
from speccon.Ktail import KTail 
from speccon.SEKT_Ktail import SEKT_TAIL
from speccon.ContractorPlus import ContractorPlus
from speccon.SMCon import SMCon 
import time 
...
class ConMiner:
    ...
    def readPreProcessedSliceInvariant(self, invariant):
        ...
        json.dump(self.pre_invariants, open(os.path.join(self.workdir, self.contractName+".pre_inv"), "w"), indent=4)
        json.dump(self.post_invariants, open(os.path.join(self.workdir, self.contractName+".post_inv"), "w"), indent=4)
    ...
    def addTraces(self, trace_slice_file, percentage: float = None):
        ...
        if percentage is not None:
            assert isinstance(percentage, float) and percentage > 0 and percentage < 1.0
        ...
        for trace_slice in trace_slices:
            ...
            for index in range(len(trace_slice["event_trace"])):
                ...
                if len(trace_slice["state_trace"]) == len(trace_slice["event_trace"]):
                    ...
                    assert len(trace_slice["state_trace"]) == len(trace_slice["event_trace"]) + 1
    ...
    def contractorplus(self):
        contractor = ContractorPlus(list(self.fieldPredMapping.keys()), self.pre_invariants, self.post_invariants)
        contractor.contractor()
        output_file = os.path.join(self.workdir, "contractorplus")
        contractor.visualize(output_file)
    
    def smcon(self):
        algo = SMCon(self.workdir, self.initialState, self.fields, self.pre_invariants, self.post_invariants, self.traces)
        fsm = algo.smcon() 
        return fsm 
...
def _main(workdir, contract_name, slice_invariant_file, slice_configuration_file, trace_slice_file):
    ...
    slicer = InvariantSlice(slice_configuration=slice_configuration_file, invariant_file=slice_invariant_file)
    invariants = slicer.get_invariant_slice()
    json.dump(invariants, open(os.path.join(workdir, contract_name+"." + "invariant-preprocessed.json"), "w"), indent=4)
...
def test():
    ...
    #     pool.starmap(test_project, zip(repeat(workdir), projects, repeat(invariant_dir), repeat(trace_dir)))
        test_project(workdir=workdir, project=project, invariant_dir=invariant_dir, trace_dir=trace_dir)
...
def main(workdir, address, contractName, invariant_file, slice_configuration_file, trace_slice_file):
    ...
    miner = ConMiner(workdir=os.path.join(workdir, address), contractName=contractName)
