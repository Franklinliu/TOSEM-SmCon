import logging
from speccon.specAutomata import *
from speccon.utils.graph import Graph, Node
...
class StateAutomata(Graph):
    ...
    def visualize(self, workdir):
        ...
        for from_state_id in self.transitions:
            from_state = list(filter(lambda x: x.unique_id == from_state_id, self.states))[0]
            ...
            for label in self.transitions[from_state_id]:
                for to_state_id in self.transitions[from_state_id][label]:
                    to_state = list(filter(lambda x: x.unique_id == to_state_id, self.states))[0]
                    ...
                    string_transitions[from_state_index][_label].add(to_state_index)

        for from_state_index in string_transitions:
            for label in string_transitions[from_state_index]:
                string_transitions[from_state_index][label] = list(string_transitions[from_state_index][label])
        ...
        json.dump(result, open(f"{workdir}/FSM-{StateAutomata.printCount}.json", "w"))
        ...
        def applyTrace(cur, trace, statetrace):
            ...
            if len(trace) == 0:
                ...
                for i in range(0, len(statetrace)-2, 2):
                    visited_transitions.add((statetrace[i], statetrace[i+1], statetrace[i+2]))
            ...
            for next_state_id in self.transitions[cur.unique_id][method]:
                nextState: StateNode = list(filter(lambda state: state.unique_id == next_state_id, self.states))[0]
            ...
            for label in self.transitions[from_state_id]:
                for to_state_id in self.transitions[from_state_id][label]:
                    if (from_state_id, label, to_state_id) not in visited_transitions:
                        unvisited_transitions.add((from_state_id, label, to_state_id))
        
        for transition in unvisited_transitions:
            from_state_id, label, to_state_id = transition[0], transition[1], transition[2]
            self.removeTransition(from_index=from_state_id, label=label, to_index=to_state_id)

        unvisited_states = list(filter(lambda state: state.unique_id not in visited_states, self.states))
        logging.warning("unvisited transition {}".format(unvisited_transitions))
        logging.warning("unvisited states {}".format([state.predicate for state in unvisited_states]))
    ...
    def checkSpuriousPath(self):
        ...
        for state_trace in new_recorded_all_state_traces:
            for i in range(len(state_trace)-1, 1, -2):
                q_n = state_trace[i]
                assert any(map(lambda x: x.unique_id == q_n, self.states)), "invalid state trace containing unknown states"
    ...
    def splitAndRemove(self, pi_n_1):
        ...
        cur_method = pi_n_1[-2]
        q_n_state: StateNode =  list(filter(lambda x: x.unique_id == pi_n_1[-3], self.states))[0] 
...
class SMCon:
    ...
    def Init(self) -> StateAutomata:
        fsm = StateAutomata(self.statevars, self.pre_invariants, self.post_invariants, self.statetraces)
