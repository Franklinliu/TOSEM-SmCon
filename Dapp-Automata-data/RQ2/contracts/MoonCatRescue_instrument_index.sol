...
contract MoonCatRescue {
  ...
  function nameCat(bytes5 catId, bytes32 catName) onlyCatOwner(catId) {
    ...
    require(catNames[cat_indexed] == 0x0); // ensure the current name is empty; cats can only be named once
    require(!adoptionOffers[cat_indexed].exists); // cats cannot be named while they are up for adoption
    catNames[cat_indexed] = catName;
    CatNamed(cat_indexed, catName);
  }
  ...
  function makeAdoptionOffer(bytes5 catId, uint price) onlyCatOwner(catId) {
    require(catId == cat_indexed);
    require(price > 0);
    adoptionOffers[cat_indexed] = AdoptionOffer(true, cat_indexed, msg.sender, price, 0x0);
    AdoptionOffered(cat_indexed, price, 0x0);
  }
  ...
  function makeAdoptionOfferToAddress(bytes5 catId, uint price, address to) onlyCatOwner(catId) isNotSender(to){
    require(catId == cat_indexed);
    adoptionOffers[cat_indexed] = AdoptionOffer(true, cat_indexed, msg.sender, price, to);
    AdoptionOffered(cat_indexed, price, to);
  }

  /* cancel an adoption offer */
  function cancelAdoptionOffer(bytes5 catId) onlyCatOwner(catId) {
    require(catId == cat_indexed);
    adoptionOffers[cat_indexed] = AdoptionOffer(false, cat_indexed, 0x0, 0, 0x0);
    AdoptionOfferCancelled(cat_indexed);
  }

  /* accepts an adoption offer  */
  function acceptAdoptionOffer(bytes5 catId) payable {
    require(catId == cat_indexed);
    AdoptionOffer storage offer = adoptionOffers[cat_indexed];
    ...
    transferCat(cat_indexed, catOwners[cat_indexed], msg.sender, offer.price);
  }

  /* transfer a cat directly without payment */
  function giveCat(bytes5 catId, address to) onlyCatOwner(cat_indexed) {
    require(catId == cat_indexed);
    transferCat(cat_indexed, msg.sender, to, 0);
  }

  /* requests adoption of a cat with an ETH offer */
  function makeAdoptionRequest(bytes5 catId) payable isNotSender(catOwners[cat_indexed]) {
    require(catId == cat_indexed);
    require(catOwners[cat_indexed] != 0x0); // the cat must be owned
    AdoptionRequest storage existingRequest = adoptionRequests[cat_indexed];
    require(msg.value > 0);
    ...
    adoptionRequests[cat_indexed] = AdoptionRequest(true, cat_indexed, msg.sender, msg.value);
    AdoptionRequested(cat_indexed, msg.value, msg.sender);

  }

  /* allows the owner of the cat to accept an adoption request */
  function acceptAdoptionRequest(bytes5 catId) onlyCatOwner(cat_indexed) {
    require(catId == cat_indexed);
    AdoptionRequest storage existingRequest = adoptionRequests[cat_indexed];
    ...
    adoptionRequests[cat_indexed] = AdoptionRequest(false, cat_indexed, 0x0, 0); // the adoption request must be cancelled before calling transferCat to prevent refunding the requester.
    transferCat(cat_indexed, msg.sender, existingRequester, existingPrice);
  }
  ...
  function cancelAdoptionRequest(bytes5 catId) {
    ...
    adoptionRequests[cat_indexed] = AdoptionRequest(false, cat_indexed, 0x0, 0);
